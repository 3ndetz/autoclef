...
public class Settings implements IFailableConfigFile {
     ...
     * <p>
     * NOT RECOMMENDED, as it will make debugging more difficult. But if you know what you're doing, go nuts.
     */
    ...
    /**
     * If true, use Minecraft's crafting recipe book to place items into the crafting table (should be much faster as
     * it's almost instant)
     * <p>
    ...
    /**
     * minimum amount of food to have in the inventory. if we have less food than this value the bot will go pickup some
     * more.
     */
    ...
    /**
     * amount of food to collect when the food in inventory is lower than the value of minimumFoodAllowed
     */
     ...
     * <p>
     * If the bot is collecting a resource and finds a chest within this range, it will grab the resource from the
     * chest.
     * <p>
     * Set this to 0 to disable chest pickups.
     * <p>
     * Don't set this too high, as the bot will prioritize chests even if the resource is easily accessible now.
     */
    private float resourceChestLocateRange = 500;

    /**
     * Some block resources are by default obtained through non-mining means. Crafting tables for example, are normally
     * crafted using planks.
     * <p>
     * However, if the block resource is found within this range it may be mined first.
     * <p>
     * Set this to 0 to disable this feature (keep in mind, this will not affect blocks like "dirt" and "cobblestone"
     * that can only be obtained through mining)
     * <p>
     * Set this to -1 to ALWAYS mine a block if it's catalogued. This is not recommended. For example, if the bot
     * happens to track a crafting table 10000 blocks away, and it then tries obtaining one it will travel 10000 blocks
     * to mine that table, even if it finds itself in a forest where the wood is abundant.
     */
    private float resourceMineRange = 100;

    /**
     * When going to the nearest chest to store items, the bot may normally dig up dungeons constantly. If this is set
     * to true, the bot will search around each chest to make sure it's not in a dungeon.
     */
     ...
     * <p>
     * Will help with navigation as sometimes dropped items will be underground, but this behaviour only makes sense in
     * regular minecraft worlds.
     */
    ...
    /**
     * Uses killaura to move mobs away and performs survival moves including: - Running away from hostile mobs when your
     * health is low - Run away from creepers about to blow up - Avoid wither skeletons and other really dangerous mobs
     * - Attempting to dodge arrows and other projectiles
     */
    private boolean mobDefense = true;

    /**
     * Defines how force field behaves when "mobDefense" is set to true. Note that the force field is not here to KILL
     * mobs, but PUSH THEM AWAY.
     * <p>
     * <p>
     * Strategies:
     * <p>
     * FASTEST: All hostiles are attacked at every possible moment, every frame. DELAY: Closest hostile is attacked with
     * a sword when your attack is charged up SMART: Closest hostile is attacked at max every 0.2 seconds. OFF: Off
     */
     ...
     * <p>
     * Baritone doesn't know how to move underwater so this should cause no problems, but disable it if you want the bot
     * to be able to sink.
     */
    private boolean avoidDrowning = true;

    /**
     * If enabled, the bot will close the open screen (furnace/crafting/chest/whatever) when the bot detects that its
     * look direction has changed OR that it is mining something.
     * <p>
    ...
    /**
     * This setting lets you configure what the bot should do if it needs to go to the nether but can't find a nether
     * portal immediately.
     * <p>
     * Options: BUILD_PORTAL_VANILLA: Builds a nether portal, either with obsidian or with a water bucket and lava pool.
    ...
    /**
     * When fast traveling via the nether, walk to our destination if we somehow end up closer than this range in the
     * overworld. We will normally travel well within this range (to within 100 blocks if not within a few), so keep
     * this value ~decently~ large.
     */
     ...
     * <p>
     * For example, try setting this to "idle" to make the bot continue surviving/eating/escaping mobs. Or "follow <Your
    ...
    /**
     * If set, will run this command after death. Also {deathmessage} will be replaced with the death message.
     * <p>
     * For example, try setting this to "@goto <Your base coords>" to make the bot go to the base before continuing the
     * task it was given. Or setting it to "I died!" will send the i died message Or setting it to "/back" will execute
     * /back on the server When running altoclef commands, if the prefix is not @, the set prefix should be used for the
     * command and not @ You may run multiple commands even commands of differet type by splitting them with " & ".
     * Example: /home & i died with message: {deathmessage} & @get diamond
     */
    ...
    private List<Item> throwawayItems = Streams.concat(
            Stream.of(
                    ...
                    Items.GRAVEL,
                    Items.COBBLED_DEEPSLATE,
                    Items.ACACIA_LEAVES, Items.BIRCH_LEAVES, Items.DARK_OAK_LEAVES, Items.OAK_LEAVES,
                    Items.JUNGLE_LEAVES, Items.SPRUCE_LEAVES,
// Nether junk, to be fair it's mostly tuned for the "beat game" task
    ...
    /**
     * If true, items with custom names will be protected/marked as "important" so they won't be thrown away.
     */
    private boolean dontThrowAwayCustomNameItems = true;
    private boolean dontThrowAwayEnchantedItems = true;

    /**
     * If we need to throw away something but we don't have any "throwaway Items", throw away any unimportant item
     * that's not currently needed in our task chain.
     * <p>
    ...
    /**
     * We will NEVER throw away these items. Even if "throwAwayUnusedItems" is true and one of these items is not used
     * in a task.
     */
    @JsonSerialize(using = ItemSerializer.class)
    @JsonDeserialize(using = ItemDeserializer.class)
     ...
     * <p>
     * Be careful when setting this to false, as ALL burnable items are liable to be burned if they're not protected
     * (blaze rods, beds, wooden tools, crafting tables etc.)
     */
    ...
    /**
     * Where "home base" is for the bot. Some tasks use this value if you tell them to, but don't worry about changing
     * this unless you NEED it.
     */
    private BlockPos homeBasePosition = new BlockPos(0, 64, 0);

    /**
     * These areas will not be mined. Used to prevent griefing, or to define a "spawn protection" zone so the bot
     * doesn't keep trying to break spawn protected blocks.
     * <p>
     * Example: protects two areas. A "spawn" area from (x=-10 z=-10) to (x=10 z=10) and a home base at around (x =
     * 1100, y = 2050)
     * <p>
     * areasToProtect : [ { start: "-10, 0, -10", end: "10, 255, 10" }, { start: "1000, 50, 2000", end: "1200, 255,
     * 2100" }, ],
     */
     ...
     * Java gateway port.
     * <p>
     * Python gateway port will be set to "YourJavaGatewayPort plus 1", default to 25334
     */
    ...
    public Item[] getThrowawayItems(AltoClef mod, boolean includeProtected) {
        return throwawayItems.stream().filter(item -> includeProtected || !mod.getBehaviour().isProtected(item))
                .toArray(Item[]::new);
    }
