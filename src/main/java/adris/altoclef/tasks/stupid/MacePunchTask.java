...
public class MacePunchTask extends Task {
    ...
    public static boolean canMacePunch(AltoClef mod, Vec3d pos) {
        ...
        // For 3 height enough dist is 9 blocks
        // 9 block
        // for 40 height enough dist is 15 blocks
        // ~37 height blocks ~equals 6 horizontal blocks distance
        // -> 6 height blocks -> 1 horizontal block

        // Check if our vertical advantage allows us to reach the target horizontally
        // For every 6 blocks of height, we can reach 1 block horizontally
        // The +6 provides an initial boost to account for jump mechanics
        double HEIGHT_BLOCKS_PER_HORIZONTAL = 6;

        // this value is definitely based on minecraft forward sprint speed
        // with standart, without any effects.]
        // TODO should be calculated based on attribute entity velocity + potion effects

        // converting vertical to horizontal
        // double ratio = MACE_HIT_REACH + yDiff / HEIGHT_BLOCKS_PER_HORIZONTAL;
        double ratio = MACE_HIT_REACH + HEIGHT_BLOCKS_PER_HORIZONTAL - (MACE_Y_MIN_DIFF - yDiff) / HEIGHT_BLOCKS_PER_HORIZONTAL;
        // ratio need to be 9 for yDiff = 3 and HEIGHT_BLOCKS_PER_HORIZONTAL = 6 and reach 3
        // so, 3 -> 9
        // 40 -> 15
        //Debug.logMessage("Can perform mace punch: height=" + yDiff + ", distance=" + xzDist +
        //        ", ratio=" + ratio);
        return ratio >= xzDist;
    }
    ...
    protected Task onTick(AltoClef mod) {
        ...
        // Preparing. We should have MACE in invertory
        if (!mod.getItemStorage().hasItemInventoryOnly(Items.MACE)) {
        ...
        if (_jumped && !_maceTimeout.elapsed() && !mod.getPlayer().isOnGround()) {
            ...
            if (LookHelper.canHitEntity(mod, _target)) {
                ...
                if (LookHelper.isLookingAtEntity(mod, _target)) {
                    ...
                    mod.getControllerExtras().attack(_target, false);
                    String targetString;
        ...
        if(MACE_Y != 0) {
        //if (!canMacePunch(mod, _target.getPos())) {
            // Step 1.
            // Get in range with target
            // first get to XY range
            // if (!mod.getPlayer().isInRange(_target, MACE_XZ_RANGE)) {
            //     return new GetToXZTask(_target.getBlockX(), _target.getBlockZ());
            // }
            // Step 2. Get to Y range (need height difference)

            // 40 blocks vertical ~-> 10 blocks horizontal
            if (yDiff < MACE_Y) {
        ...
        mod.getSlotHandler().forceEquipItem(Items.MACE);
        // 3.2 Perform JUMP
        // TODO 3.2.1 perform 1s timeout shift + back move to prepare acceleration
        // TODO 3.2.2 perform RUN and jump only when on the edge of block
